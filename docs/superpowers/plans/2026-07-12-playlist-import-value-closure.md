...
计数边界固定为 parser 完成空行过滤和语义去重后的 `ImportedSong.id` 集合，而不是 OCR 原始行数或最终 track ID。为每个 stable importedSong ID 建立且只建立一个 `PlaylistReconciliationEntry`；outcome 用单一枚举表达互斥状态，至少包含：

```swift
public enum PlaylistReconciliationOutcome: Codable, Equatable, Sendable {
    case reviewRemoved
    case selectedOriginal(planItemID: UUID, trackID: String)                 // X
    case selectedAdoptedAlternative(planItemID: UUID, trackID: String)       // R
...
测试必须证明两个不同导入 ID 可以命中同一个 track：其中至多一个条目携带最终 planItemID，另一个因语义去重进入 `.verifiedNotSelected(..., reason: .semanticDuplicate)`，不能因为 trackID 相同而丢掉一条导入记录。另建 alias 归并、Live/原版版本归并和容量截断 fixture，逐条断言 outcome 与解释原因。
...
XCTAssertEqual(ledger.entry(for: importedSongs[8].id)?.outcome, .pending)
XCTAssertEqual(ledger.summary.formalPlanCount, 6)
```

红灯还要覆盖：重复 importedSong ID、遗漏 ID、额外 ID、一个导入条目试图对应两个 outcome、重复 planItemID，以及 selected/supplement planItem 集合与正式 plan.items 不同。
...
Expected: FAIL，错误明确指向 `PlaylistReconciliationEntry`、`PlaylistReconciliationLedger` 或逐条 outcome 不存在；不得把已有的 `N=X+R+Y` 纯计数测试误记为本步骤通过。

- [ ] **Step 3: 实现以 stable importedSong ID 唯一的完整账本**

在 `Models.swift` 增加 `PlaylistReconciliationEntry`、`PlaylistReconciliationOutcome`、`PlaylistSupplementEntry`、`PlaylistReconciliationLedger` 和可枚举的 `PlaylistReconciliationError`。entry 的唯一身份永远是 importedSong stable ID；resolved trackID、semantic key、alias 或版本归并结果只能作为 outcome 证据，不能取代导入身份。
...
activeReviewedCount = entries.count - reviewRemoved.count
activeReviewedCount = verified.count + pending.count + unmatched.count
verified.count = selectedOriginal(X).count + selectedAdoptedAlternative(R).count + verifiedNotSelected(Q).count
formalPlanCount(N) = selectedOriginal(X).count + selectedAdoptedAlternative(R).count + supplements(Y).count
```

快照保存完整 entries 和 supplements。旧快照没有逐条账本时，不能从汇总数字反向创造条目；恢复为 stale 并要求重新分析，直到能从原始 importedSong IDs、完整 matches 和 plan items 重建并校验。

- [ ] **Step 4: 在匹配和计划原子发布点拒绝错账**

`prepareForReview` 保留 importedSong stable ID 与原始身份快照。整理删除把对应 entry 设为 `.reviewRemoved`；匹配逐 importedSong ID 写入 accepted/pending/unmatched 证据；计划生成在语义去重和容量截断完成后，才把 verified 条目分别归入 X、R 或 Q，并把非导入的正式 items 写成 supplements Y。

发布点校验：entries 的 ID 集合与原始 importedSong ID 集合完全相等；selected entry 和 supplement 的 planItemID 集合与正式 plan.items ID 集合完全相等；每个 planItemID 只出现一次。任一不一致都保留上一份稳定结果并显示可重试错误。
...
500/1000 首只渲染 summary 和按需分页的 entries。UI 测试覆盖整理删除、两个导入条目命中同一 track、alias/版本归并、容量截断、恢复后重建和故意错账不发布；页面 summary 必须读取 ledger 派生结果，不能反向手填计数。
...
Expected: 全部 PASS；10/100/500/1000 的每个 importedSong stable ID 都有且只有一个可解释 outcome；同 track 多条导入、alias/版本归并和容量截断不丢条目；故意错账时上一份 ready 计划不被覆盖。
...
同时增加纯领域测试：锁定歌曲删除被拒绝时不得消费已有 `lastRemovedTrackUndo`；成功单条恢复或全部恢复各只增加一次 `trackControlsRevision`，并只启动一次重排。结果页移除/恢复前后，Task 17 ledger、`reviewRemovedEntries`、`removedDuringReviewCount` 和 `reviewRevision` 必须逐值相等；整理页删除/撤销也不得改变 `planControlRemovedRecords`。
...
UI 测试必须真实执行：从结果页移除 12 首、终止并重启、加载更多、恢复第 9 首、确认 plan-control 计数从 12 变 11、全部恢复后卡片消失。每次结果页移除/恢复只能修改 `planControlRemovedRecords`、增加一次 `trackControlsRevision`、把 plan 转 stale 并触发一次重排；绝不修改 Task 17 的 `reviewRemovedEntries`、`removedDuringReviewCount` 或 `reviewRevision`。迟到的旧重排不得重新写回已恢复 tombstone。
...
Expected: 全部 PASS；第 9 首可跨重启单独恢复，未知旧记录可清除，恢复操作不会重复增加 revision 或发布迟到计划；两套删除记录在全部测试中互不变化。
...
同步设计稿 5.2、8.1 与 11，明确最多 20 张、OCR 并发最多 2、批次原子发布、部分失败必须显示 `m/n` 与失败序号，以及多截图为早期闭环和公开发布共同门禁。
...
- [ ] **Step 1: 写 SHA 隔离证据、真实平台回执与 clean-tree 红灯**

fixture 要求唯一证据根为 `Build/PublicReleaseEvidence/<RELEASE_SHA>/`，并绑定 clean tree、xcarchive、IPA SHA-256、Bundle/version/build、签名团队、Task 23、Task 24A/24B、iOS 17 smoke、App Store Connect 原始 upload 回执、真实 build resource ID、`processingState`、`buildBetaDetails` 状态和 TestFlight 安装记录。拒绝手填 build ID/状态、错误 SHA 目录、缺原始 JSON/UTC 时间戳、冻结后跟踪文件变化或证据引用其他 commit。
...
- [ ] **Step 3: 实现 Archive/Export、API 查询和只读证据校验**

`archive_public_release.sh` 只向传入的 `EVIDENCE_DIR` 写 xcarchive、IPA、哈希和命令日志；`fetch_app_store_build_receipt.py` 使用 App Store Connect API JWT 查询与 IPA CFBundleVersion 完全一致的 build，并保存 builds 原始 JSON、buildBetaDetails 原始 JSON、查询 UTC 时间戳、候选 SHA 和 IPA SHA-256。`validate_public_release.py` 只读校验这些产物，要求 `processingState == VALID` 且 TestFlight 状态来自原始 API，不接受人工填写替代。
...
该 commit 是冻结前最后一次跟踪文件变更。冻结后所有重跑结果、截图、QA 填写和平台回执只能写入 `$EVIDENCE_DIR`；禁止修改 `docs/`、基线截图或任何跟踪文件。若必须修文档或代码，废弃整个证据目录，提交后生成新 SHA 并从头重跑。
...
Run:
    ...
    RELEASE_SHA="$(git rev-parse HEAD)"
    EVIDENCE_DIR="Build/PublicReleaseEvidence/$RELEASE_SHA"
    RELEASE_SHA="$RELEASE_SHA" EVIDENCE_DIR="$EVIDENCE_DIR" ./scripts/archive_public_release.sh
...
不得因认证、网络、处理或业务校验失败而自动换工具重试；fallback 只处理已安装 Xcode 确认不支持 altool 的情况，并在证据中记录唯一 `uploadTool`。任一上传命令无平台成功回执即 BLOCKED。
...
脚本通过 `GET /v1/builds` 按 app 与 CFBundleVersion 查找真实 build resource ID，再查询对应 `buildBetaDetails`；原始响应分别保存为 `app-store-builds.json` 和 `build-beta-details.json`，另写不可手填的 receipt metadata（UTC 时间戳、SHA、IPA hash、HTTP 状态）。没有唯一 build、`processingState` 不是 `VALID`、TestFlight 状态未就绪、API 不可达或回执与 IPA 不符时保持 BLOCKED。
...
- [ ] **Step 8: 在冻结 SHA 上只写 Build 证据重跑最终全故事**

重跑 Task 16/22A 的全量测试、两套字号截图与真机 smoke，以及 Task 17–24B 全部聚焦门禁，但使用：

    RESULT_BUNDLE="$EVIDENCE_DIR/FullUI.xcresult" ./scripts/run_ui_tests.sh
    RESULT_BUNDLE="$EVIDENCE_DIR/Screenshots.xcresult" CONTENT_SIZE=large SCREENSHOT_DIR="$EVIDENCE_DIR/screenshots" ./scripts/capture_ui_test_screenshots.sh
    RESULT_BUNDLE="$EVIDENCE_DIR/Screenshots-AXXXL.xcresult" CONTENT_SIZE=accessibility-extra-extra-extra-large SCREENSHOT_DIR="$EVIDENCE_DIR/screenshots-large-text" ./scripts/capture_ui_test_screenshots.sh
...
App Store Connect 元数据、privacy/support URL、App Privacy 问卷、加密说明、第三方数据保留复核和人工设备结果使用 `PUBLIC_RELEASE_QA.md` 的冻结模板写入 `$EVIDENCE_DIR/public-release-qa.md`，不回写仓库。最后运行：
    ...
    test -z "$(git status --porcelain --untracked-files=all)"
    python3 scripts/validate_public_release.py --evidence-dir "$EVIDENCE_DIR" --expected-commit "$RELEASE_SHA" --require-ios-major 17

Expected: 三条命令成功，原始 upload/build/TestFlight 回执和人工证据完整。无真实平台回执、任何跟踪文件变化或任一外部门禁未通过时，允许的最高结论仍是“早期验证版核心闭环”。
...
- 每个 importedSong stable ID 都有且只有一个 reconciliation outcome，同曲命中、alias/版本归并和容量截断均可逐条解释；summary 只从 entries/supplements 派生。
...
- 多截图批量导入最多 20 张，部分失败明确、批次原子发布，真机 3 张重叠截图 smoke 已通过；本轮没有单图收窄后的延期分支。
...
- Task 25 在同一冻结 commit 上完成 Distribution Archive/Export、唯一工具上传、App Store Connect API 真实 build/processing/TestFlight 回执、TestFlight 安装升级、最低 iOS 17 smoke、元数据与隐私问卷。
- Task 16/22A 与 Task 17–24B 的最终自动化、截图、真机和人工证据全部只写入 `Build/PublicReleaseEvidence/<RELEASE_SHA>/` 并绑定同一冻结 SHA；冻结后仓库跟踪文件没有变化。
