...
3. 匹配完成未排歌：原曲/替代分别为 `.verifiedOriginal/.verifiedAdoptedAlternative + .notGenerated`，其余为 pending/unmatched。
4. plan stale：`reviewMatchOutcome` 不变，`planDisposition` 保留上一次成功 plan basis 下的 X/R/Q 供 stale 解释，但导出和开唱由 plan state 禁用。
5. 重排成功：在新 plan/basis 校验通过时，一次性替换全部 `planDisposition` 与 supplements；失败或取消保持旧 stale dispositions。
...
红灯还要覆盖：重复 importedSong ID、遗漏/额外 ID、负数或重复 playlist order、非法 outcome/disposition 组合、重复 planItemID，以及 selected/supplement planItem 集合与正式 plan.items 不同。
...
`PlaylistPreparationSummary` 永远从 `reviewMatchOutcome` 派生；`planSummary` 在所有 entries 都是 `.notGenerated` 时为 `nil`，不能把“尚未排歌”显示成 X/R/Q 均为零。ready/stale 的 plan summary 从上一次成功 plan basis 对应的 dispositions/supplements 派生。快照保存双阶段 entries、supplements 和 disposition plan basis；旧快照不能证明时恢复为 stale 并重新分析，不从汇总数字反造条目。

- [ ] **Step 4: 在匹配和计划原子发布点拒绝错账**

`prepareForReview` 保留 stable ID、原始顺序与身份快照。整理删除/撤销或歌名编辑只更新 `reviewMatchOutcome` 并使 `planDisposition=.notGenerated`；完整匹配原子提交 verified/pending/unmatched，仍保持 `.notGenerated`。计划生成在语义去重、锁定和容量截断完成后，按固定 tie-break 计算新的 X/R/Q 与 supplements Y；只有 plan、basis、全量 dispositions 和 supplements 全部校验成功才一次发布。

反馈、场景、音区、`planControlRemovedRecords` 和锁定等 plan controls 只增加对应 revision、把 plan 转 stale，不改 `reviewMatchOutcome`，也不提前改 `planDisposition`。成功重排才原子替换后者；失败保留旧 stale 映射，便于解释“上次方案”，但所有输出门继续关闭。
...
500/1000 首只渲染 summary 和按需分页的 entries。UI/领域测试覆盖五阶段、整理删除、两个导入条目命中同一 track、输入顺序随机化、alias/版本归并、锁定、容量截断、stale 重排失败/成功和恢复后重建；页面 summary 必须读取双阶段 ledger，不能反向手填计数。
...
Expected: 全部 PASS；每个 importedSong stable ID 都有稳定 reviewMatchOutcome 与当前/上次成功 planDisposition；五阶段均可表达，plan controls 不污染匹配真相，同 track 归属在随机输入、锁定和重排后仍确定；故意错账不覆盖 stable/stale 计划。
...
fixture 要求先存在只读 `Build/PublicReleaseCandidate/candidate.json`。它必须记录冻结 `releaseSHA`、由 `git write-tree` 得到且与 `HEAD^{tree}` 相等的 `treeHash`、基线证据摘要、Bundle Identifier、`marketingVersion` 和 `buildNumber`；后续唯一证据根只能是 `Build/PublicReleaseEvidence/<releaseSHA>/`。基线证据摘要定义为：对已跟踪的 `docs/screenshots/**`、`docs/screenshots-large-text/**`、`docs/PUBLIC_RELEASE_QA.md`、`docs/MANUAL_QA.md`、`docs/QUALITY_AUDIT.md`、`README.md` 按 UTF-8 路径字节序排序，串联每个“路径 + NUL + 文件 SHA-256 + LF”后再做 SHA-256。缺文件、未跟踪文件或摘要漂移都失败。

证据还必须绑定 clean tree、完整 frozen-gates manifest、每条 Swift/Python/UI/截图/Task 24B 命令及退出码、xcarchive、IPA SHA-256、Bundle/version/build、签名团队、Task 23、Task 24A/24B、iOS 17 smoke、App Store Connect 原始 upload 回执、真实 build resource ID、`processingState`、`buildBetaDetails` 状态和 TestFlight 安装记录。fixture 必须拒绝：候选清单可写或字段缺失、当前 HEAD/tree 与清单不符、漏跑命令/缺日志/非零退出码、手填 build ID/状态、错误 SHA 目录、缺原始 JSON/UTC 时间戳、冻结后跟踪文件变化、证据引用其他 commit、ASC 查询零条或多条，以及 marketing version、build number、App ID 或 IPA SHA-256 任一不一致。
...
`fetch_app_store_build_receipt.py` 使用 App Store Connect API JWT：先按 `ASC_APP_ID + preReleaseVersion.version == marketingVersion` 查询唯一 pre-release version，再按 `ASC_APP_ID + preReleaseVersion ID + build.version == buildNumber` 查询唯一 build，最后查询对应 `buildBetaDetails`。原始响应分别保存为 `app-store-pre-release-versions.json`、`app-store-builds.json` 和 `build-beta-details.json`。receipt metadata 必须验证并记录 `(ASC_APP_ID, marketingVersion, buildNumber)` 元组、IPA SHA-256、候选 SHA/tree、唯一 build resource ID、HTTP 状态、查询 UTC 和 `processingState`；任一步零条或多条都保持 BLOCKED。`validate_public_release.py` 要求 `processingState == VALID` 且 TestFlight 状态来自原始 API，不接受人工填写替代。
...
脚本先从 IPA 实读 Bundle Identifier、`CFBundleShortVersionString`、`CFBundleVersion` 并重新计算 IPA SHA-256，拒绝与 `ipa-metadata.json` 或候选清单不一致。随后通过 `GET /v1/preReleaseVersions` 按 `ASC_APP_ID + preReleaseVersion.version == marketingVersion` 获取唯一 pre-release version，再通过 `GET /v1/builds` 按 `ASC_APP_ID + preReleaseVersion ID + build.version == buildNumber` 获取唯一真实 build resource ID，最后查询对应 `buildBetaDetails`。原始响应分别保存为 `app-store-pre-release-versions.json`、`app-store-builds.json` 和 `build-beta-details.json`；receipt metadata 绑定 `(ASC_APP_ID, marketingVersion, buildNumber)`、IPA SHA-256、候选 SHA/tree、唯一 build ID、UTC 与 HTTP 状态。任一查询零条或多条、`processingState` 不是 `VALID`、TestFlight 状态未就绪、API 不可达或元组/hash 不符时保持 BLOCKED。
...
runner 的固定 required-command 集合至少包含：`swift test`；delivery/public-release Python 单测与全部 Python validator；固定 DerivedData 的 generic iOS Simulator Release build 加 `validate_release.py <app-bundle>`；Task 17–24B 聚焦 iOS 测试；完整 `run_ui_tests.sh`；普通字号与 accessibility-extra-extra-extra-large 两套截图；`./scripts/validate.sh`；以及 `validate_catalog.py --release --write-gap-report "$EVIDENCE_DIR/catalog/CATALOG_COVERAGE_GAP_REPORT.md"`。Task 16/22A 全量门禁、Task 17–24B 聚焦门禁、两套截图和最低 iOS 17 smoke 均须出现在 `frozen-gates-manifest.json`；任何命令都不得使用写入 `docs/` 的参数。
...
- Task 25 由只读 `Build/PublicReleaseCandidate/candidate.json` 绑定冻结 commit/tree、基线证据摘要和 Bundle/marketing/build，并在同一候选上完成 Distribution Archive/Export、唯一工具上传、App Store Connect API 唯一 `(app ID, marketing version, build number)` build/processing/TestFlight 回执、TestFlight 安装升级、最低 iOS 17 smoke、元数据与隐私问卷。
