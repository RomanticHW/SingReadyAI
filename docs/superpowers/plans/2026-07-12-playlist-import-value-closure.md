...
- [x] **Step 4: 将聚合器改为 raw candidate 语义去重**
...
- [x] **Step 5: 验证 Codable、稳定顺序和正式计划隔离**
...
- [x] **Step 1: 为 basis 相等性与失效矩阵写红灯测试**
...
`catalogRevision` 由当前本地曲库的稳定 ID、标题、歌手、版本身份，以及会影响匹配、排歌或导出结果的正式元数据共同生成，不使用进程随机 `Hasher`；集合型字段先排序，浮点字段使用稳定 bit pattern。场景、音区和内容指纹同样使用稳定串或固定摘要。`voiceFingerprint` 必须包含实际音区结果，`voiceSource` 单独保存，防止“通用参考”和“实测音区”结果碰巧相同却被视为同一 basis。

- [x] **Step 4: 实现完整分析、准备摘要和显式状态类型**
...
只有 `persistedAnalysis?.basis == currentBasis`，且 playlist、完整 review 草稿与 analysis 中的歌曲 ID、内容和数量逐项一致时才返回 `.ready(persistedAnalysis.basis)`；空结果、缺项、重复项或陈旧歌曲一律 `notStarted`。运行中的临时 progress 不进入参数、不持久化；不能根据导航 stage 推断 ready。
...
- [x] **Step 2: 运行聚焦测试，确认当前先清空内存的实现失败**
...
- [x] **Step 3: 给 actor 增加带 generation 的稳定提交**
...
- [x] **Step 4: 将 `prepareForReview` 拆成候选构造与提交**
...
- Modify: `SingReadyAI/App/DesignSystem/DesignSystemComponents.swift`
...
- [x] **Step 2: 运行聚焦测试，确认当前分散字段无法保证一致**
...
- [x] **Step 3: Store 使用单一 completedAnalysis**
...
- [x] **Step 6: 新增进度 UI fixture 与 UI 测试**

新增 `-singreadyLargeMixedReview` 和可控慢分析 fixture。断言：

- 整理页显示“已处理 n/总数 首”，n 单调增加。
- 取消后仍停留整理页，草稿数量不变。
...
- [x] **Step 2: 运行 workflow 测试，确认当前同步赋值红灯**
...
- [x] **Step 3: 构造稳定 PlanBasis 并在后台生成**
...
- [x] **Step 4: 将确认、采用和所有上游变化统一转 stale**
...
- [x] **Step 9: 提交原子计划生成**
    ...
    git add \
      ...
      SingReadyAI/App/DesignSystem/DesignSystemComponents.swift \
      ...
      Tests/SingReadyAISharedKitTests/WorkflowPersistenceExecutorTests.swift \
      UITests/SingReadyAIUITests/SingReadyAIUITests.swift \
      docs/superpowers/plans/2026-07-12-playlist-import-value-closure.md
...
- [x] **Step 1: 写 v1 迁移、v2 回环和 16MB 红灯**
...
- [x] **Step 2: 运行存储测试，确认 v2 壳仍缺完整迁移与写保护**
...
- [x] **Step 4: 同批迁移所有 externalCandidateTracks 运行时引用**
...
- [x] **Step 2: 运行聚焦 UI 测试，确认当前 ForEach 全量渲染红灯**
...
- [x] **Step 2: 运行现有 match report UI 测试，确认旧流程红灯**
...
- [x] **Step 2: 运行 exporter 和结果页测试，确认旧 provisional 分区红灯**
...
- [x] **Step 4: 更新结果页来源摘要和卡片**

`userFacingPlanSummary` 直接读取 `generationSummary`。`SongRecommendationCard` 使用 `item.origin.displayName`，不再根据 catalogSource 或 reason 猜来源。`ExternalCandidateCollectionCard` 放在正式时间线前，避免被长歌单埋没；E 仍独立展示且不计入 N。
