...
系统只自动采用歌名或维护别名完全一致且歌手身份匹配的歌曲；缺歌手、相近匹配和替代建议进入异常区。用户可以先按已确认内容排歌，异常项稍后再处理。
...
2. **自动处理正常项**：歌名身份和歌手身份同时匹配的结果自动进入可用集合，不要求逐首点击。
...
| --- | --- | --- | --- |
| `acceptedOriginalExact` | 规范化歌名或已维护别名完全一致，并且导入歌手与维护的歌手别名规则匹配 | 已核对 | 是，来源为原歌单 |
| `acceptedOriginalConfirmed` | 用户从同名或相近候选中明确确认原曲身份 | 已核对 | 是，来源为原歌单 |
| `identityConfirmationRequired` | 缺少歌手；同名存在一个或多个候选；或未达到完全一致但相似度不低于 0.78 | 待确认 | 否 |
...
自动采用只允许 `acceptedOriginalExact`。即使本地参考中只有一个同名候选，只要导入内容缺少歌手，也必须进入 `identityConfirmationRequired`；它可以被跳过，但不能被静默当成原曲。相似度阈值只负责候选分组，不足以单独形成确定性身份结论。
...
当前 215 首本地参考曲库继续作为演唱属性和推荐规则的验证数据。本规格只修正它在导入、匹配、推荐和结果解释中的使用合同，不在同一实施计划中增加曲库行数，也不使用公开元数据自动伪造难度、音区、高音风险、合唱分数或场景标签。

曲库扩充是后续独立子项目，必须先单独确认目标数量、歌曲范围、数据来源与使用权、演唱属性维护方式和完成标准，再进入实施。该子项目至少遵循：
...
这样把“产品闭环是否可用”和“数据集扩充到多少首”拆成两个可分别完成、分别验收的交付，不让开放式数据收集阻塞本轮核心修复。
...
`SongPlanGenerationSummary` 构造时必须断言 `正式计划总数 = 原歌单入选数 + 用户采用替代歌曲数 + 补充推荐数`。不满足时不得发布计划。`ExternalCandidateCollection.count` 只作为 E 单独展示。
...
1. **新导入**：链接读取、文本解析或 OCR 都先写入临时结果；当前稳定工作流在新歌单成功前保持不变。只有“非空有效歌单 + 整理草稿”同时准备好并成功保存后，才原子替换当前歌单并清除旧匹配、画像、计划、外部候选、锁定和移除。失败、取消、超时或进程终止均丢弃临时结果，不影响旧工作流。
...
5. **反馈、锁定和移除**：操作先更新本机状态和修订，再触发重排。重排成功后替换计划；失败时保留已记录的用户选择，并明确提示计划尚未更新。
...
- 单张截图不承诺识别数百首歌曲；长歌单优先引导使用公开链接或文本，图片过大、像素超限或识别失败时不冻结页面。
...
- 歌名或维护别名完全一致且歌手身份匹配的歌曲自动处理；其他候选按 5.4 的状态规则进入待确认或暂未找到，不会被批量强制确认。
...
- 本地参考曲库的数据量扩充；它将使用独立规格定义目标数量、来源与属性校验。
- 云端账号、跨设备同步和多人实时协作。
- 现场播放、切歌或点歌队列控制。
